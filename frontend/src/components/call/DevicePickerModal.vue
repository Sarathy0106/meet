<template>
  ...
  >
    <div class="w-full max-w-lg bg-[#202124] border border-meet-border rounded-2xl shadow-2xl overflow-hidden flex flex-col">
      <!-- Modal Header -->
      <div class="px-6 py-4 border-b border-meet-border flex items-center justify-between">
        <h2 class="text-lg font-medium text-white">Audio & Video Settings</h2>
        <button
          @click="close"
      ...
      <div class="px-6 py-4 border-t border-meet-border bg-[#1a1b1e] flex justify-end">
        <button
          @click="close"
          class="px-5 py-2 bg-meet-blue hover:bg-meet-blue-hover text-white text-sm font-medium rounded-full transition-colors"
        >
          Done
        </button>
      </div>
